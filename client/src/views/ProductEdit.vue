<template>
    ...
    >
        <NavBar />
        <h1 class="text-5xl text-center font-bold p-4">Edition d'une photo</h1>
        <div
        ...
        >
            ...
            <button class="" type="button" @click="deleteProduct">
                <img
                    class="h-6"
...
export default {
    ...
    methods: {
        updateProduct() {
            PhotoDataService.update(this.id, this.product).then(() => {
        ...
        deleteProduct() {
            PhotoDataService.delete(this.id).then(() => {
    ...
    mounted() {
        PhotoDataService.get(this.id).then((response) => {
